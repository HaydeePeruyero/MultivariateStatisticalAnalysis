...
La regla de , fue propuesta por Kaiser en 1959. Propone retener únicamente los componentes cuyos eigenvalores sean mayores que 1 (cuando el PCA se realiza sobre la **matriz de correlaciones**).
...
**a) Contribuciones por componente:** Las contribuciones indican qué porcentaje del total de varianza explicada por un componente proviene de cada variable.
...
**b) Mapa de calidad de representación ($\cos^2$):** El valor $\cos^2$ es la calidad de representación de una variable o individuo en el plano principal (usualmente PC1–PC2).
...
a) ¿Por qué sería conveniente aplicar PCA a esta base de datos? ¿Es importante estandarizar los datos?

b) ¿Cuántas componentes principales serán necesarios para el análisis?

c) ¿Cuál es la variable que más contribuye en cada eje? ¿Cuál es la que menos contribuye?

d) ¿Qué representa el primer eje? ¿Qué nombre le asignaría? ¿Qué representa el segundo eje?

e) ¿Cuál es el individuo mejor representado en el primer plano factorial? ¿Cuál es la peor?

f) ¿Cómo podemos representar los niveles en los datos en las dos primeras componentes?

Ejercicio tomado de https://rpubs.com/JairoAyala/PCA.


**Ejercicio 3:** En este ejercicio trabajarás con el conjunto de datos Glass Identification, disponible en [datos](https://github.com/HaydeePeruyero/MultivariateStatisticalAnalysis/tree/main/data/glass_identification_full.csv) y [descripciones](https://github.com/HaydeePeruyero/MultivariateStatisticalAnalysis/tree/main/data/glass_identification_variables.csv) . Este dataset contiene mediciones químicas sobre muestras de vidrio, con el objetivo original de clasificar cada muestra según su tipo (ventanas, recipientes, vidrio flotado, etc.).
...
Además, contiene una variable categórica `Type` que identifica el tipo de vidrio (1–7). Este set de datos fue descargado de [UCI Machine Learning Repository](https://archive.ics.uci.edu/dataset/42/glass+identification).

Aplica un análisis de componentes principales (PCA) para:

a) Explorar la variabilidad del conjunto de datos.

b) Detarmina si se deben estandarizar o no los datos.
