...
\]
Sin embargo, los elementos *fuera de la diagonal* de $S$ normalmente no se reproducen exactamente por $\widetilde{L}\,\widetilde{L}^\top + \widetilde{\Psi}$.
...
**Ejemplo(Ej 9.3 Johnson and Wichern):** Se encuestó a una muestra de clientes para calificar, en una escala de 7 puntos,
...
La proporción de varianza total explicada por estos dos factores es $\frac{\hat\lambda_1 + \hat\lambda_2}{p}=$`r sum(eig$values[1:2])/5`.
...
**Proposición: ** Sea $X_1, X_2, \ldots, X_n$ una muestra aleatoria de $\mathcal{N}_p(\mu,\Sigma)$,
donde
\[
\Sigma = LL^{\top} + \Psi
\]
...
2) La proporción de la varianza total de la muestra explicada por el
    factor $j$ es
    \[
    \frac{
