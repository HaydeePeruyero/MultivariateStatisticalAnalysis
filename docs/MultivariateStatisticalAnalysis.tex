...
\item
  Los \(\lambda_{ji}\) se llaman también las cargas (\emph{loadings}) de la j-ésima variable en el factor i-ésimo.
\item
...
Por tanto, las cargas \(\lambda_{ij}\) pueden interpretarse como la \textbf{covarianza} entre la variable original \(X_i\) y el factor \(f_j\).
...
\textbf{Ejemplo:} Verificar que \(Σ= \Lambda\Lambda^\top+\Psi\).
...
\subsection{Factores principales iterados}\label{factores-principales-iterados}
